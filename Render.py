...
def reflectVector(normal, dirVector):
    ...
    reflect = multiplicarPorEscalar(reflect, normal)
    reflect = restarVectores(reflect, dirVector)
    reflect = normalizado(reflect)
    return reflect
...
def refractVector(N, I, ior):
    ...
    if cosi < 0:
        ...
        etai, etat = etat, etai
        N = multiplicarPorEscalar(-1,N)
    eta = etai/etat
    k = 1 - eta * eta * (1 - (cosi * cosi))

    if k < 0: # Total Internal Reflection
        return None
    R = sumarVectores((multiplicarPorEscalar(eta , I)) , multiplicarPorEscalar(eta*cosi-k**0.5 , N) )
    R = normalizado(R)
    return R 
...
class Raytracer(object):
    ...
    def rtRender(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                direction = normalizado(direction)
    ...
    def castRay(self, orig, direction, origObj = None, recursion = 0):
        ...
        finalColor = [0,0,0]
        # Direccion de vista
        view_dir = restarVectores(self.camPosition, intersect.point)
        view_dir = normalizado( view_dir)
        ...
        if self.dirLight:
            ...
            light_dir = multiplicarPorEscalar(-1 , self.dirLight.direction)
            ...
            coef = max(0, dot(view_dir, reflect)) ** material.spec
            spec_intensity = self.dirLight.intensity * coef
            specColor = [
            ...
            if shadInter is not None:
                shadow_intensity = 1
            dirLightColor = multiplicarPorEscalar((1 - shadow_intensity) , sumarVectores(diffuseColor , specColor))
        ...
        for pointLight in self.pointLights:
            ...
            shadow_intensity = 0
            # Sacamos la direccion de la luz para este punto
            light_dir = restarVectores(pointLight.position, intersect.point)
            light_dir = normalizado(light_dir)
            # Calculamos el valor del diffuse color
            ...
            shadMat, shadInter = self.scene_intercept(intersect.point,  light_dir, intersect.sceneObject)
            if shadInter is not None and shadInter.distance < norma(restarVectores(pointLight.position, intersect.point)):
                shadow_intensity = 1
            pLightColor = sumarVectores( pLightColor, multiplicarPorEscalar((1 - shadow_intensity) , (sumarVectores(diffuseColor , specColor))))

        
        if material.matType == OPAQUE:
            # Formula de iluminacion, PHONG
            finalColor = sumarVectores(sumarVectores(ambientColor , dirLightColor) , pLightColor)
            if material.texture and intersect.texCoords:
                ...
                finalColor = multiplicarVectores(finalColor , [texColor[2] / 255, texColor[1] / 255, texColor[0] / 255] )
        elif material.matType == REFLECTIVE:
            reflect = reflectVector(intersect.normal, multiplicarPorEscalar(-1 , direction))
            ...
            bias = multiplicarPorEscalar(0.001 ,intersect.normal)
            kr = fresnel(intersect.normal, direction, material.ior)

            reflect = reflectVector(intersect.normal, multiplicarPorEscalar(-1 , direction))
            reflectOrig = sumarVectores(intersect.point, bias) if outside else restarVectores(intersect.point, bias)
            ...
            if kr < 1:
                ...
                refractOrig = restarVectores(intersect.point, bias) if outside else sumarVectores(intersect.point, bias)
            ...
            finalColor = sumarVectores( multiplicarPorEscalar(kr , reflectColor) , multiplicarPorEscalar((1-kr) , refractColor))
        # Le aplicamos el color del objeto
        finalColor = multiplicarVectores( finalColor , objectColor)
